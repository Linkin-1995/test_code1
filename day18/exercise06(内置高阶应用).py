...
# 2.
for item in map(lambda emp: (emp.eid, emp.name), list_employee):
    print(item)
...
# 5. map( lambda , filter(lambda,列表) )
for name in map(lambda e: e.name, filter(lambda element: element.money > 15000, list_employee)):
    print(name)
